...
This repository contains the `pet` package, which is a minimalistic template engine written in Python.
...
As you can see that way a PET document is nothing but a preprocessed Markdown, Asciidoc, APT or just any markup format, where you can mix the Python code into the text.
...
```
pet README2.md.pet README2.md
```

will generate the `README2.md`, which is an output file in this case.
You should never edit or modify the output file manually, as you may lose the changes when executing `pet` next time.
...
If you intend to use a different delimiter pair, you can modify your copy of `src/pet/processor.py`.
