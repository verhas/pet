...
[![Version](https://img.shields.io/badge/version-2.0.0-orange)](pyproject.toml)
...
| `snippet(source, exclude=None)` | Scan for named `snippet` / `end snippet` markers; `source` is a directory, glob pattern, or list of either; `exclude` accepts a filename, glob, or list |
